...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      checkboxGroupInput(inputId = "workclass", label = "Select class of occupation",
                         choices = names(table(d$workclass)), inline = TRUE),
...
server <- function(input, output) {
  ...
  output$show_plot <- renderPlot({
    ...
    d <- d %>% filter(workclass %in% input$workclass)
